...
Closed when `scripts/first-sol.mjs` stays green (leak → repair → power → sleep → still → ice → water → drink → crop). Ice must be scanned before it fuels the still; soil must be scanned before the last potato goes in the dirt. After that, deepen **stabilize** (storm → solar → battery → heater/grid → crops; storms scar the array and can snap the roof cable; dead grid takes the still offline — no flask, no tank trickle, no phantom kW fighting dawn — but leftover tank sips remain, and a full flask is still collectable; a blackout home bleeds pressure until the suit will not refill inside; a cold heater holds a damaged-array blackout through noon — shed load, do not wait for the sun; heater-off to save that night also cools the greenhouse; the bunk is not a magic O₂ tank when the house cannot refill the suit). Fetching splice wire is a daylight walk, not a storm-night stroll. Pathfinder is a longer leash than the solar wreck; placing the radio is not Hello, Earth. A clear-day sleep can finish the listen; a storm pauses that tape, it does not rewind it; the journal waits for the reply. Suit range is O₂ ∩ warmth ∩ thirst ∩ hunger — a dry mouth is not a full tank; leftover tank sips pack the walk. After Earth, the MAV walk wants a still flask and a potato that is not the last seed — a desk sip is thirst, not cargo; the Hab locker is a crate, not your hands. The scanner names a wreck you can walk to, not the horizon, and does not print a 48 m loot inventory, light the far wreck, or label the Pathfinder radio from the Hab. Rover waits.
