...
- Coupled stabilize (harness): storm cuts solar and crop light; grid death freezes plots **and** takes the still offline (no phantom kW on a dead grid) while leftover tank sips and a full flask still work; storm scars the array and can snap the roof cable (wire from the solar wreck). Sleep-through-storm is a diagnosis, not a silent kW drop
- Seal patch visual
