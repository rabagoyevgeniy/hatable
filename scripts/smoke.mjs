...
import { noteScan, createScience, lootBeaconVisible, pickScanTarget, canFuelStill, canPlantCrop, canUseWire, radioCanListen, RADIO_CONTACT_S, canBuildRadio, recipeKnown, LOOT_RING_RANGE, labLines, overlayNamesOutpost } from "../src/systems/science.js";
...
{
  const blank = { science: createScience() };
  must(!overlayNamesOutpost(blank, "pathfinder", 40, { scanning: true }), "hold-F is not a Pathfinder atlas");
  must(!overlayNamesOutpost(blank, "mav", 200, { scanning: true }), "hold-F is not a MAV atlas");
  must(overlayNamesOutpost(blank, "pathfinder", LOOT_RING_RANGE - 2), "ident-range overlay may name the lander");
  must(overlayNamesOutpost(blank, "hab", 40, { scanning: true }), "Hab overlay is home, not a scan");
  must(overlayNamesOutpost({ science: { known: { pathfinder: true } } }, "pathfinder", 40, { scanning: true }), "identified Pathfinder may stay named at range");
}
